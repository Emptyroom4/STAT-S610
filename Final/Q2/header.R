...
draw_beta = function(data,state){
  z = rinvGauss(1, 1/(state$tau*abs(data$y-(data$x%*%state$beta))), 1)
  mean = 1/(state$tau*abs(data$y-(data$x%*%state$beta)))
  sigma = diag(z, length(mean))
  return(rmvnorm(1, mean, sigma))
}
